...
docker exec $INPUT_CONTAINER_NAME mysql -u root --password="$INPUT_MYSQL_PASSWORD" -D $INPUT_MYSQL_DATABASE -h mysql --port=$INPUT_CONTAINER_PORT --protocol=TCP "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%';"
